    ...
    >
      ...
      <ul class="flex flex-col h-auto overflow-y-auto custom-scrollbar">
        ...
        <li v-for="notification in notifications" :key="notification.id">
          <a class="flex gap-3 rounded-lg border-b border-gray-100 p-3 hover:bg-gray-100 dark:border-gray-800 dark:hover:bg-white/5" href="#">
            ...
            <span class="block">
              <span class="mb-1 block text-theme-sm text-gray-500">
                ...
                {{ notification.action }} 
                <span class="font-bold text-danger-500">{{ notification.project }}</span>
              </span>
              <span class="text-theme-xs text-gray-400">{{ notification.time }}</span>
            </span>
          </a>
        </li>
      </ul>
...
const esCocinero = computed(() => rolUsuario.value === 'Cocinero')

const cargarPedidosPreparacion = async () => {
  if (!esCocinero.value) return;

  try {
    // Asumiendo que tienes un endpoint que filtre por estado
    const res = await API.get('/restrik/notificacionpedidos');
    if (res.data.data) {
      // Mapeamos los datos de tu tabla pedidos al formato de la lista
      notifications.value = res.data.data.map(pedido => ({
        id: pedido.id_pedido,
        userName: `La Mesa ${pedido.codigo_mesa}`,
        action: 'ha realizado un nuevo pedido',
        type: 'Cocina',
        time: formatearHora(pedido.fecha_pedido),
      }));

      // Si hay nuevos pedidos, activamos el punto naranja
      if (notifications.value.length > 0) {
        notifying.value = true;
      }
    }
  } catch (error) {
    console.error("Error cargando notificaciones de cocina:", error);
  }
}
...
onMounted(() => {
  ...
  if (esCocinero.value) {
    ...
    // Opcional: Polling cada 30 segundos para nuevas órdenes
    const interval = setInterval(cargarPedidosPreparacion, 30000);
